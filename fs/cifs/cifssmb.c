...
static void convert_ace_to_cifs_ace_legacy(struct cifs_posix_ace *cifs_ace,
...
static __u16 ACL_to_cifs_posix_legacy(char *parm_data, const char *pACL,
				      const int buflen, const int acl_type)
{
	...
	for (i = 0; i < count; i++)
		convert_ace_to_cifs_ace_legacy(&cifs_acl->ace_array[i], &ace[i]);
	if (rc == 0) {
...
int
CIFSSMBSetPosixACLLegacy(const unsigned int xid, struct cifs_tcon *tcon,
...
setAclRetry:
	...
	data_count = ACL_to_cifs_posix_legacy(parm_data, local_acl, buflen,
					      acl_type);
...
/**
 * cifs_convert_ace - convert ACL from cifs to POSIX ACL format
 ...
 */
static void cifs_convert_ace(struct posix_acl_entry *ace,
			     struct cifs_posix_ace *cifs_ace)
{
	/* u8 cifs fields do not need le conversion */
	ace->e_perm = cpu_to_le16(cifs_ace->cifs_e_perm);
...
/**
 * cifs_copy_posix_acl - copy cifs ACL format to POSIX ACL format
 ...
 */
static int cifs_copy_posix_acl(struct posix_acl **acl, char *src,
...
int cifs_get_acl(const unsigned int xid, struct cifs_tcon *tcon,
...
queryAclRetry:
	 ...
	 * allocated @acl in cifs_copy_posix_acl() we are guaranteed to return
...
/**
 * convert_ace_to_cifs_ace - convert ACL entry from POSIX ACL to cifs format
...
/**
 * ACL_to_cifs_posix - convert ACLs from POSIX ACL to cifs format
 ...
 */
static __u16 ACL_to_cifs_posix(char *parm_data, const struct posix_acl *acl,
			       const int acl_type)
{
	__u16 rc = 0;
	struct cifs_posix_acl *cifs_acl = (struct cifs_posix_acl *)parm_data;
...
setAclRetry:
	...
	/* convert to on the wire format for POSIX ACL */
	data_count = ACL_to_cifs_posix(parm_data, acl, acl_type);
