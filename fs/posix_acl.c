			    ...
			    int type)
{
	...
	/*
	 * Normally, the ACL returned by ->get_{dentry_}acl will be cached.
	 * A filesystem can prevent that by calling
	 * forget_cached_acl(inode, type) in ->get_{dentry_}acl.
	 *
	 * If the filesystem doesn't have a get_{dentry_}acl() function at all,
	 * we'll just create the negative cache entry.
	 */
	if (dentry) {
		if (!inode->i_op->get_dentry_acl) {
			set_cached_acl(inode, type, NULL);
			return NULL;
		}
		acl = inode->i_op->get_dentry_acl(mnt_userns, dentry, type);
	} else {
...
get_dentry_acl(struct user_namespace *mnt_userns, struct dentry *dentry,
	       int type)
{
	return __get_acl(mnt_userns, dentry, d_inode(dentry), type);
}
...
retry_deleg:
	...
	if (!error) {
		...
		evm_inode_post_set_acl(dentry, acl_name);
	}
			      ...
			      struct dentry *dentry, const char *acl_name)
{
	...
	acl = get_dentry_acl(mnt_userns, dentry, acl_type);
	if (IS_ERR(acl))
		return acl;
	if (!acl)
